    ...
    <v-container>
        <v-card>
            <v-container grid-list-md mb-0>
                <h2 class="text-md-center">REPORT TRANSAKSI LAYANAN</h2>
                <v-layout row wrap style="margin:10px">
                    ...
                    <v-flex xs6 class="text-right">
                        ...
                        <v-select class="align-center" :items="transaksi_layanan_status" v-model="filter" label="Filter Kategori Produk" outlined clearable="true"></v-select>
                    </v-flex>
                </v-layout>
                <template>
                                    ...
                                    <v-card class="mx-auto" max-width="750" outlined>
                                        <v-list-item three-line>
                                            <v-layout>
                                                <v-flex>
                                                    <v-list-item-content>
                                                        <v-list-item-title style="font-size:22px">{{ item.id }}</v-list-item-title>
                                                       
                                                        <v-list-item-title style="font-size:22px">Nomor Transaksi = {{ item.no_transaksi }} </v-list-item-title>
                                                        <v-list-item-title style="font-size:22px">Nomor Telpon = {{ item.no_telp }} </v-list-item-title>
                                                ...
                                                <v-flex>
                                                    <v-list-item-content>
                                                        <v-list-item-subtitle class="font-weight-light align-end">Status</v-list-item-subtitle>
                                                        
                                                        <v-list-item-subtitle style="font-size:18px" class="font-weight-regular align-end">{{ item.status }}</v-list-item-subtitle>
                                                    </v-list-item-content>
                                                </v-flex>
                                                <v-flex xs1>
...
export default {
    ...
    methods: {
        getData() {
            var uri = this.$apiUrl + '/TransaksiLayanan/admin'
         ...
         getJenisLayanan(){             
            var uri = this.$apiUrl + '/TransaksiLayanan/admin'             
            this.$http.get(uri).then(response =>{
                this.transaksi_layanan=response.data.message;
                this.transaksi_layanan_status=response.data.message.map(x => x.status);
            })
        },
