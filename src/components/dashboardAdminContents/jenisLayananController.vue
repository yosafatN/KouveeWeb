    ...
    <v-container>
        <v-card>
            <v-container grid-list-md mb-0>
                <h2 class="text-md-center">Jenis Layanan</h2>
        ...
        <v-dialog v-model="dialog" persistent max-width="600px">
            <v-card>
                <v-card-title>
                    <span class="headline">JenisLayanan</span>
                </v-card-title>
                <v-card-text>
                    <v-container>
                        <v-row>
...
export default {    
    ...
    methods:{         
        ...
        updateData(){             
            ...
            var uri = this.$apiUrl + '/JenisLayanan/' + this.updatedId;             
            this.load = true             
            this.$http.post(uri,this.request).then(response =>{ 
                this.snackbar = true; //mengaktifkan snackbar               
                this.color = 'green'; //memberi warna snackbar               
        ...
        deleteData(deleteId){ //mengahapus data
            var uri = this.$apiUrl + '/JenisLayanan/delete/' + deleteId; //data dihapus berdasarkan id 
            this.request.append('updated_by', 'Ajeng9999');
            this.$http.post(uri,this.request).then(response =>{                 
                this.snackbar = true;                 
